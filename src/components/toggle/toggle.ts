...
/** Only supports radio button mode currently. */
@Directive({
  selector: 'md-toggle-group',
  host: {
    'role': 'radiogroup',
  },
})
...
export class MdToggle implements OnInit {
  ...
  /** The parent toggle group. Can exist without a toggleGroup. */
  ...
  constructor(@Optional() toggleGroup: MdToggleGroup, public toggleDispatcher: MdToggleDispatcher) {
  ...
  /** Input change handler, called with keyboard selection. */
  onInputChange() {
    this.checked = true;
  }
